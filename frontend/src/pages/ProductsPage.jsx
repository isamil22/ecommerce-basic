...
const ProductDetailPage = () => {
    ...
    // v-- 2. Add this entire block --v
    useEffect(() => {
        if (product) {
            ReactGA.event({
                category: "Ecommerce",
                action: "view_item",
                label: product.name, // Sends the name of the viewed product
            });
        }
    }, [product]); // This ensures the event fires only when the product is loaded
